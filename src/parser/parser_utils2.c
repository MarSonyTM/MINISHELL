...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/03 14:04:29 by mafurnic          #+#    #+#             */
/*   Updated: 2024/04/03 14:19:22 by mafurnic         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_cmd **cmd, t_env *env, int *arg_count)
{
	...
	*current = (*current)->next;
	if (*current == NULL || ((*current)->type != TOKEN_COMMAND
...
int	dispatch_token_handler(t_token **current, t_cmd **cmd, t_env *env, t_cmd **current_cmd, int *arg_count)
{
	t_token_type	type;

	type = (*current)->type;
	if (type == TOKEN_BUILTIN || type == TOKEN_COMMAND)
		handle_builtin_or_command(cmd, *current, env, current_cmd, arg_count);
	else if (type == TOKEN_ARG && *current_cmd != NULL)
		handle_argument(*current_cmd, *current);
	else if (type == TOKEN_INPUT && *current_cmd != NULL)
		handle_input(*current_cmd, *current);
	else if (type == TOKEN_REDIRECT_OUT || type == TOKEN_REDIRECT_OUT_APPEND)
	...
	{
		if (!handle_pipe_token(current, cmd, env, arg_count))
			return (1);
	}
	return (0);
}
...
int	process_tokens(t_token *tokens, t_cmd **cmd, t_env *env)
{
	...
	while (current)
	{
		if (dispatch_token_handler(&current, cmd, env,

...
int process_redirect_out_append(char currentChar, char *buffer, int *bufIndex,int inQuote,  t_token **tokens, int *TokenCount, t_env *env, int *i) {
    (void)currentChar;
    if (*bufIndex > 0) {
...
int lexer(char *input, t_token **tokens, t_env *env) 
{
    ...
    while ((currentChar = input[i]) != '\0' && !quote_error) {
        if (is_whitespace(currentChar) && inQuote == 0) {
            ...
            process_redirect_out_append(currentChar, buffer, &bufIndex, inQuote, tokens, &TokenCount, env, &i);
        } else {
    ...
    if (bufIndex > 0 && !quote_error) {
        if (inQuote != 0) { // If we ended in a quote, it's an unclosed quote error
            error(ERR_QUOT, NULL);
